 ...
 *
 * Мир: море с севера, пляж ~180 блоков в ширину и ~80 вглубь (дальше поля), с трёх сторон поля с обрывами.
 *   - северный край карты (мировой бордер, z = -2000) — дно моря (10 блоков воды);
 *   - урез воды около z = -1395 и совпадает с линией обрывов — пляж не выдаётся в море;
 *   - пляж сужается к воде (бухта), вглубь полого поднимается в поля, по краям обрывы.
 *
...
public final class TeyvatOceanEdge {
    ...
    /** Номинальный урез воды: профиль пляжа пересекает уровень моря при g = 0.39
     *  (реально извивается шумом) — там же начинаются и обрывы по краям бухты. */
    public static final int WATERLINE_Z = SHORE_Z + (int) Math.round(SEA_WIDTH * 0.39);
