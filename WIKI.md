# Teyvat — Википедия мода

**Teyvat** — глобальный мод для *Minecraft* (Fabric 1.21.10), переносящий механики, мир и атмосферу игры *Genshin Impact* в блочный мир. Мод включает боевую систему с комбо и заряженными атаками, систему выносливости, точки телепортации, энциклопедию путешественника, спутницу Паймон, процедурную генерацию биомов Тейвата и полноэкранную карту исследованной территории.
...
2. [Битва](#битва)
   ...
   - [Водяной снаряд](#водяной-снаряд)
   - [Урон от падения](#урон-от-падения)
3. [Сокровища](#сокровища)
   - [Мора](#мора)
   - [Слизь слайма](#слизь-слайма)
   - [Концентрат слайма](#концентрат-слайма)
...
На границе пляжа и равнин генерируется стартовая точка телепортации — ромб из каменной кладки с красной плитой в центре и колонной над ней.
...
В холодном климате точки телепортации служат источником тепла, обогревая окрестности.
...
Точка телепортации состоит из:
- **Плита** (красная/синяя) — основание, на котором стоит колонна;
- **Основание колонны** — расширяющаяся ступенчатая платформа;
- **Ствол** — тонкая колонна 6×6 пикселей;
- **Капитель** — инвертированная пирамида, венчающая конструкцию.

Все элементы построены из воксельных моделей с текстурами камня.
...
Во время атаки движение заблокировано: герой двигается только микро-рывком по направлению атаки. Прыжок прерывает серию (как в Genshin Impact).
...
Гидро слаймы атакуют, выплёвывая водяные снаряды. Они появляются во время тренировки с Паймон и в мире как обычные мобы.

### Водяной снаряд

Сфера воды, которую выплёвывает гидро слайм.

Когда слайм выплёвывает сферу, он на миг замирает — это лучший момент для удара. Снаряд летит по прямой линии и наносит урон при попадании.
...
Концентрат слайма — редчайшая слизь, за которую многое дадут.

Выпадает только со слаймов 60 уровня и выше. Ценный материал для продвинутых улучшений.
...
- Победы над врагами;
- Выполненные задания Паймон;
- Активацию точек телепортации (+50).
...
Обучение состоит из семи заданий:

1. Знакомство с Паймон
2. Колесо мыши (приближение)
3. Клавиша C (осмотр)
4. Бег (двойное W)
5. Рывок (Left Ctrl)
6. Атака (ЛКМ)
7. Подбор предметов (F)

После выполнения всех заданий Паймон прощается и выпускает игрока в свободное исследование мира. Нажмите **X**, чтобы пропустить обучение полностью.

---

## Карта путешественника

Полноэкранная карта (**M**) показывает всю исследованную территорию в стиле карты Genshin Impact.
...
Энциклопедия (**N**) — внутриигровой справочник в стиле архива Genshin Impact. Разделён на четыре раздела:

| Раздел | Цвет | Содержание |
|--------|------|-----------|
| Мир | бирюзовый | Локации, структуры |
| Битва | красный | Оружие, боевые механики, монстры |
| Сокровища | золотой | Материалы, валюта |
| Приключения | голубой | Навигация, система прогрессии |

Записи открываются при первой встрече с соответствующим содержимым. После урока Паймон запись дополняется полной версией. Неоткрытые записи не отображаются.
...
- **Загрузчик**: Fabric
- **Совместимость**: Sodium, Iris
- **Пространство имён**: `teyvat`

### Установка

1. Скопируйте `teyvat.jar` в папку `mods/`;
2. Установите ресурс-пак Teyvat;
3. Установите шейдер TeyvatShader (форк Complementary Reimagined);
4. Запустите игру.

### Обновление

Запустите `scripts/update.sh`, затем нажмите F3+T (ресурс-пак) и F3+R (шейдер). После обновления Java-кода нужен полный перезапуск игры.
