    ...
    #define INCLUDE_SKY_COLORS
        ...
        vec3 noonUpSkyColor     = pow(skyColorM, vec3(2.9)) * (vec3(0.97, 1.0, 0.86) + rainFactor * nuscWeatherM);
        vec3 noonMiddleSkyColor = pow(skyColorM, vec3(1.5)) * (vec3(1.42, 1.36, 1.30) + rainFactor * (nmscWeatherM + nmscRainM + nmscSnowM + nmscDryM))
                                + noonUpSkyColor * 0.65;
        vec3 noonDownSkyColor   = skyColorM * (vec3(1.02, 1.0, 0.95) + rainFactor * (ndscWeatherM + ndscRainM + ndscSnowM + ndscDryM))
                                + noonUpSkyColor * 0.25;
        ...
        vec3 sunsetDownSkyColorP  = vec3(1.55, 0.92, 0.6) - vec3(0.8, 0.3, 0.0) * rainFactor;
