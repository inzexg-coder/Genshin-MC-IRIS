    ...
    #define INCLUDE_SKY_COLORS
        ...
        vec3 noonUpSkyColor     = pow(skyColorM, vec3(2.9)) * (vec3(0.92, 1.0, 0.90) + rainFactor * nuscWeatherM);
        vec3 noonMiddleSkyColor = pow(skyColorM, vec3(1.5)) * (vec3(1.42, 1.36, 1.30) + rainFactor * (nmscWeatherM + nmscRainM + nmscSnowM + nmscDryM))
                                + noonUpSkyColor * 0.65;
        vec3 noonDownSkyColor   = skyColorM * (vec3(1.10, 1.04, 0.94) + rainFactor * (ndscWeatherM + ndscRainM + ndscSnowM + ndscDryM))
                                + noonUpSkyColor * 0.25;

        vec3 sunsetUpSkyColor     = skyColorM2 * (vec3(0.74, 0.54, 0.50) + vec3(0.1, 0.2, 0.35) * rainFactor2);
        vec3 sunsetMiddleSkyColor = skyColorM2 * (vec3(1.9, 1.36, 1.14) + vec3(0.15, 0.25, -0.05) * rainFactor2);
        vec3 sunsetDownSkyColorP  = vec3(1.62, 0.96, 0.62) - vec3(0.8, 0.3, 0.0) * rainFactor;
        ...
        vec3 nightColFactor      = 0.9 * vec3(0.06, 0.115, 0.27) * (1.0 - 0.5 * rainFactor) + skyColor;
