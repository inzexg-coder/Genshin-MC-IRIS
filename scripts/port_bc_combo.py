...
Что скрипт чинит сверх исходников:
  * голова ВСЕГДА смотрит по направлению удара, но в физиологичном диапазоне
    (yaw -0.55..0.75, pitch -0.35..0.3): каналы головы прописаны отдельной
    кривой для каждого удара, голова следит за клинком и НЕ «осматривает
    стороны» (никаких абсолютных поворотов на 2π);
...
# Значения НАМЕРЕННО маленькие (физиологичный диапазон): голова следит за
# клинком, но не «осматривает стороны» — никаких абсолютных поворотов на 2π.
# Стыки согласованы: конец N = начало N+1 (и конец 5 = начало 1 для цикла).
HEADS = {
    # 1: слева направо — голова ведёт клинок от замаха слева до сопровождения справа
    "hit1": head_kf([
        (0.00, 0.00, 0.00),
        (0.22, 0.45, 0.04),
        (0.36, 0.20, 0.00),
    ...
    # 2: апперкот справа снизу вверх влево — вниз-вправо на замахе, вверх-влево в финале
    "hit2": head_kf([
        (0.00, -0.40, 0.16),
        (0.22, -0.50, 0.26),
        (0.36, -0.25, 0.10),
    ...
    # 3: разворот — голова ведёт поворот, но остаётся в малом диапазоне
    "hit3": head_kf([
        (0.00, 0.45, -0.30),
        (0.22, 0.30, -0.18),
        (0.44, 0.10, -0.08),
    ...
    # 4: справа налево — зеркально удару 1
    "hit4": head_kf([
        (0.00, -0.35, -0.12),
        (0.22, -0.50, 0.06),
        (0.36, -0.20, 0.04),
