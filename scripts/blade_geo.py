...
ГРИП-КОРРЕКТИРОВКА (CombatController.BLADE_GRIP_C): чтобы рука и меч были ОДНОЙ
ПРЯМОЙ (~180°), лезвие направляется ТОЧНО по линии «плечо→кисть». В системе
руки это направление normalize(G·O), где G = Rx(-90)·Ry(180), O = (1, 6, -9.5) px
— смещение точки хвата от пивота плеча (translate(1/16, 0.125, -0.625) +
display translate(0, 4, 0.5)). В игре предмет доворачивается
(R_frame = D·C·Q·D⁻¹), чтобы клинок шёл по этой прямой (C: Y -> D⁻¹·Ô,
Q: поворот плоскости лезвия вокруг оси клинка). Числа: angle=0.100668672 rad,
axis=(-0.882085226, 0, 0.471089858) (проверено: G·D·C·Y == G·Ô, угол 0°;
плоскость лезвия меняется не более чем на ~0.6°). Без корректировки клинок
идёт вдоль предплечья и на ударах виден тупой угол (~152°) рука-меч.
